  ...
  function submitGuess() {
    ...
    if (!/^\d{5}$/.test(guess)) {
        if (/[a-zA-Z]/.test(guess)) {
            ...
            document.getElementById("message").textContent = "Enter exactly 5 digits.";
        }
	setTimeout(() => {
...
guessInput.addEventListener("input", () => {
    guessInput.value = guessInput.value.replace(/[^0-9]/g, ""); // removes letters, spaces, special chars
});